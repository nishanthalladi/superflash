...
- To move a box into another note, change its pin's `parent` (and `x`/`y`);
  the app applies it. A note cannot end up inside itself.
...
```
{"keys":{"draw":"cmd+shift+d","select":"v","rectangle":"r","ellipse":"o","arrow":"a","line":"l",
